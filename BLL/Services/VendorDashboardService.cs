...
public class VendorDashboardService : IVendorDashboardService
{
    ...
    public async Task<byte[]> ExportTransactionHistoryAsync(ulong vendorId, DateOnly from, DateOnly to, CancellationToken cancellationToken = default)
    {
        await ValidateVendorAsync(vendorId, cancellationToken);
        ValidateDateRange(from, to);

        var transactions = await _repository.GetAllTransactionsInTimeRangeAsync(vendorId, from, to, cancellationToken);

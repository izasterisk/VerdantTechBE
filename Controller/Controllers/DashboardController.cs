...
public class DashboardController : BaseController
{
    ...
    [EndpointDescription("Có thể sử dụng cho cả Admin, Vendor và Staff. Admin/Staff sẽ thấy tổng doanh thu toàn hệ thống, Vendor chỉ thấy doanh thu của mình.")]
    ...
    [EndpointDescription("Có thể sử dụng cho cả Admin, Vendor và Staff. Admin/Staff sẽ thấy top 5 toàn hệ thống, Vendor chỉ thấy sản phẩm của mình.")]
    ...
    [EndpointDescription("Lấy thống kê số lượng đơn hàng theo từng trạng thái trong khoảng thời gian. Admin và Staff có quyền truy cập.")]
    ...
    [EndpointDescription("Lấy số lượng các yêu cầu đang chờ xử lý (VendorProfile, ProductRegistration, VendorCertificate, ProductCertificate, Request). Admin và Staff có quyền truy cập.")]
    ...
    [EndpointDescription("Có thể sử dụng cho cả Admin, Vendor và Staff. Admin/Staff sẽ thấy tổng doanh thu toàn hệ thống theo ngày, Vendor chỉ thấy doanh thu của mình.")]
    ...
    /// <summary>
    /// Lấy thống kê đánh giá sản phẩm của vendor
    /// </summary>
    /// <returns>Thống kê rating trung bình và top 3 sản phẩm có rating cao nhất/thấp nhất</returns>
    [HttpGet("products/ratings")]
    [Authorize(Roles = "Vendor")]
    [EndpointSummary("Get Products Rating Statistics")]
    [EndpointDescription("Lấy thống kê đánh giá sản phẩm của vendor bao gồm rating trung bình tất cả sản phẩm, top 3 sản phẩm có rating cao nhất và thấp nhất. Chỉ Vendor có quyền truy cập.")]
    public async Task<ActionResult<APIResponse>> GetProductsRatingStatistics()
    {
        try
        {
            var vendorId = GetCurrentUserId();
            var statistics = await _dashboardService.GetAverageRatingsByVendorIdAsync(vendorId, GetCancellationToken());
            return SuccessResponse(statistics);
        }
        catch (Exception ex)
        {
